...
def create_camera_instance(cam_meta: dict[str, Any]) -> dict[str, Any] | None:
    ...
    for attempt in range(max_retries):
        try:
            ...
            if instance:
                logger.info(f"Connecting to {cam_type} camera: {cam_id} (attempt {attempt + 1}/{max_retries})...")
            ...
            if attempt < max_retries - 1:
                logger.warning(f"Failed to connect {cam_type} camera {cam_id} (attempt {attempt + 1}/{max_retries}): {e}. Retrying...")
            else:
                logger.error(f"Failed to connect or configure {cam_type} camera {cam_id} after {max_retries} attempts: {e}")
